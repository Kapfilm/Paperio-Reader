...
constexpr uint8_t SECTION_FILE_VERSION = 69;  // near-body font-size normalization
                                              ...
                                              // v65: compact generic anchors; invalidate low-memory partial caches
...
uint32_t Section::calculatePropertyHash(const BuildParams& p) {
  ...
  if (!p.previewAnchor.empty() && p.previewMaxPages > 0) {
    for (const unsigned char c : p.previewAnchor) {
      hash ^= c;
      hash *= FNV_PRIME;
    }
    const auto* limitBytes = reinterpret_cast<const uint8_t*>(&p.previewMaxPages);
    for (size_t i = 0; i < sizeof(p.previewMaxPages); ++i) {
      hash ^= limitBytes[i];
      hash *= FNV_PRIME;
    }
  }
