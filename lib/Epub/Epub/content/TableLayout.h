#pragma once
// Shared table grid-layout packing (Phase 3 step 5, docs/parser-stage1-step5-design.md).
//
// The fused <table> path and LayoutSink both turn a set of pre-wrapped table rows into a run of
// PageTableFragment elements, greedily packing rows and page-breaking between fragments. The
// per-cell text WRAPPING (layoutAndExtractLines) is settings-dependent and stays on each caller's
// side (each already owns a renderer + fontId); this helper takes the ALREADY-WRAPPED rows plus
// the row-height/fragment math and drives page emission through a small callback context, so the
// packing logic lives once instead of two copies drifting apart. It is the table analogue of
// computeImageDisplaySize and the natural step-6 seam.
//
...
struct TablePageContext {
  ...
  // it to paragraphs + block-images (fused emitTableAsParagraphs/placeImageBlockAsBlock). The
  // helper flushes the pending fragment before invoking this so document order is preserved.
  virtual void onOversizeRow(const TableLayoutRow& row) = 0;
};

// Greedily pack `rows` into PageTableFragments, page-breaking between them. Reproduces the fused
// emitTableAsFragments packing (ChapterHtmlSlimParser.cpp:2951-3027): a fragment flushes on a
// renderCols change or when the next row would overflow the viewport; a bordered fragment/row
// adds +1px. Over-tall rows are routed to ctx.onOversizeRow after flushing the pending fragment.
