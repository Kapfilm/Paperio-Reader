...
void LayoutSink::abbreviatePreviewRuns(Block& block) const {
  ...
  const int maxAdvance = static_cast<int>(viewportWidth_) * 2;
  const int spaceAdvance = renderer_.getSpaceWidth(fontId_);

  std::vector<Word> newWords;
  std::string newText;
  newWords.reserve(block.words.size());
  std::vector<PreviewRun> keptRuns;  // preview runs no longer need marking post-abbreviation
