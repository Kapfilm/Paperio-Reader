...
BUILD_DIR="$ROOT_DIR/build/css_gaps"
BINARY="$BUILD_DIR/CssGapsTest"
...
if [ ! -d "$PLATFORMIO_DIR" ]; then
  echo "SKIP: PLATFORMIO_DIR does not exist and PlatformIO could not be located; skipping CSS gaps test." >&2
  exit 0
fi

if [ ! -d "$ARDUINO_FRAMEWORK_DIR" ]; then
  echo "SKIP: ARDUINO_FRAMEWORK_DIR does not exist: $ARDUINO_FRAMEWORK_DIR; skipping CSS gaps test." >&2
  exit 0
fi
...
SOURCES=(
  "$ROOT_DIR/test/css_gaps/CssGapsTest.cpp"
